...
public class TransportMethodsImp implements TransportMethodsInterface {
    ...
    @Override
    public ResponseEntityInterface getTransportMethods() {
        ...
        if (transportMethods.isEmpty()){
            return new ResponseEntityInterface.Builder().setSuccessful(false).setMessage("Transport Method not found").build();
        }
        Map<String, Object> data = new HashMap<>();
        data.put("transportMethods", transportMethods);

        return new ResponseEntityInterface.Builder().setSuccessful(true).setMessage("Transport Method found").setData(data).build();
    }

    @Override
    public ResponseEntityInterface createTransportMethod( String transportMethodType) {
        ...
        List <TransportMethods> transportMethods = entityManager.createQuery("SELECT t FROM TransportMethods t WHERE t.transportMethodType = :transportMethodType", TransportMethods.class)
                ...
                .getResultList();
        if (!transportMethods.isEmpty())
            return new ResponseEntityInterface.Builder().setSuccessful(false).setMessage("Transport Method already exists").build();
        ...
        return new ResponseEntityInterface.Builder().setSuccessful(true).setMessage("Transport Method created successfully").build();
    }

    @Override
    public ResponseEntityInterface updateTransportMethod( TransportMethods transportMethods) {
        ...
        entityManager.persist(transportMethod);
        return new ResponseEntityInterface.Builder().setSuccessful(true).setMessage("Transport Method updated successfully").build();
